...
class TableDefinition:
    ...
    def load_columns(self):
        ...
        for record in result:
            col = ColumnDefinition(record['column_name'], record['type'], True if record['not_null'] == 1 else False)
    ...
    def load_indexes(self):
        ...
        q = "select column_name, index_name, type, index_order from csvindexes where table_name = '" + \
            self.table_name + "'"
        result = run_q(self.cnx, q, None, fetch=True)
        index_data_temp = dict()
        ...
        for record in result:
            ...
            if index_name not in index_data_temp.keys():
                index_data_temp[index_name] = [index_type, {column_name:index_order}]
            else:
                index_data_temp[index_name][1][column_name] = index_order

        for index_name, index_data in index_data_temp.items():
            sorted_columns = list(dict(sorted(index_data[1].items(), key=lambda item:item[1])).keys())
            new_index_def = IndexDefinition(index_name, index_data[0], sorted_columns)
            self.indexes.append(new_index_def)
    ...
    def load_core_definition(self):
        ...
        result = run_q(self.cnx, q, None, fetch=True)
        self.table_name = result[0]['table_name']
    ...
    def to_json(self):
        ...
        if self.indexes is not None:
            ...
            '''
            !!!
            Had To change this so that the print works while creating the csv table
            Otherwise, it was giving errors regarding the file name
            !!!
            '''
    ...
    def save_index_definition(self, i_name, cols, type):
        ...
        print(f"Index {i_name} for {self.table_name} has been added!")
    ...
    def define_index(self, index_name, columns, type="index"):
        ...
        # Drop the indexes with the same name if already present
        self.drop_index(index_name)

        # update self.indexes with new Index Definition
        new_index = IndexDefinition(index_name, type, columns)
        self.indexes.append(new_index)

        # Update the Indexes in the SQL database
        cols_defs = []
        for col_name in columns:
            cols_defs.append(self.get_column(col_name))
        self.save_index_definition(index_name, cols_defs, type)
    ...
    def drop_index(self, index_name):
        ...
        if index_to_drop is not None:
            ...
            self.drop_indx_in_sql(index_name)  # You must implement this function
...
class CSVCatalog:
    ...
    def create_table(self, table_name, file_name, column_definitions=None, primary_key_columns=None):
        result = TableDefinition(table_name, file_name,
                                 column_definitions=column_definitions,
                                 index_definitions=primary_key_columns,
                                 cnx=self.cnx)
        return result

...
class CSVTable:
    ...
    def __get_access_path__(self, fields):
        ...
        # ************************ TO DO ***************************
        if self.__description__.indexes is None or len(self.__description__.indexes) == 0 or not self.__indexes__:
            print("No Indexes Found!!!")
            return None, None

        if fields is None or len(fields) == 0:
            print("No Fields have been inputted!!!")
            return None, None

        best_index_found = None
        for index_def_data in self.__description__.indexes:
            index_name = index_def_data.index_name
            columns = index_def_data.column_names
            if set(columns) <= set(list(fields.keys())):
                if best_index_found is None:
                    best_index_found = index_name
                else:
                    # Need to find the best one according to the selectivity criteria
                    best_index_found = self.find_best_index(best_index_found, index_name)

        if best_index_found is None:
            return None, None

        result_count = len(list(self.__indexes__[best_index_found].keys()))

        return best_index_found, result_count

    def find_best_index(self, index_1, index_2):
        index_count_1 = len(list(self.__indexes__[index_1].keys()))
        index_count_2 = len(list(self.__indexes__[index_2].keys()))

        if index_count_1 >= index_count_2:
            return index_1
        else:
            return index_2
    ...
    def __find_by_template_index__(self, t, idx_name, fields=None):
        ...
        # ************************ TO DO ***************************
        columns_in_index = []
        matching_index_def = None
        for index_def_data in self.__description__.indexes:
            index_name = index_def_data.index_name
            if index_name == idx_name:
                matching_index_def = index_def_data
                columns_in_index = index_def_data.column_names
                break

        if set(columns_in_index) <= set(list(t.keys())):
            key = self.__get_key__(matching_index_def, t)
            for column_name in columns_in_index:
                t.pop(column_name, None)
            indexed_rows = self.__indexes__[idx_name][key]
        else:
            # can't be indexed, shouldn't reach this case in ideal scenario!!!
            print("Rows can't be indexed with this set of template!!!")
            indexed_rows = self.__rows__
    ...
    def __smart_join__(self, right_r, on_fields, where_template=None, project_fields=None):
        ...
        # Optimization 1: I get the indices for the left table and find the relevant rows in the left table
        # according to the where template for the smart join query. This would filter out a lot of rows that needs
        # to be joined later with the another table and using indexes, we can find the relevant rows very quickly!
        left_r = self
        left_sub_where_template = left_r.__get_sub_where_template__(where_template)
        left_filtered_rows = left_r.__find_by_template__(left_sub_where_template)

        # Optimization 2: I get the indices for the right table and find the relevant rows in the right table
        # according to the where template for the smart join query. This would filter out a lot of rows that needs
        # to be joined later with the another table and using indexes, we can find the relevant rows very quickly!
        right_sub_where_template = right_r.__get_sub_where_template__(where_template)
        right_filtered_rows = right_r.__find_by_template__(right_sub_where_template)

        result_rows = list()
        left_rows_processed = 0

        # For appending the filtered rows from both the tables that have to  be joined...
        for lr in left_filtered_rows:
            on_template = self.__get_on_template__(lr, on_fields)
            for rr in right_filtered_rows:
                if self.matches_template(rr, on_template):
                    new_r = {**lr, **rr}  # appends two dictionaries together
                    result_rows.append(new_r)
            left_rows_processed += 1
        ...
        # This could also be done as a part of the filtering of the rows for both the tables above
        # if I had passed in a dictionary for the project fields such that I knew which
        # project fields apply to which of the tables. If I knew that, I would pass in the relevant project fields,
        # and get the projected and filtered data from both the tables before joining it! This would have
        # optimized this a little more!
        result_rows = self.project(result_rows, project_fields)
        final_joined_table = self.__table_from_rows__("Filtered JOIN(" +
                                                      left_r.__table_name__ + "," +
                                                      right_r.__table_name__ + ")", result_rows)
